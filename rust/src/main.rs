...
fn main() {
  ...
  let mut bump: bump_struct::Bump = bump_struct::Bump::new(nx, ny, mx, my);
  ...
  println!();
  println!(
    "  The bump will be generated with a height of {}",
    bump.aprof
  );
  println!();
  println!("  NX = {}", nx);
  ...
  {
    let mut f_xy = BufWriter::new(std::fs::File::create(&xy_file).unwrap());
    funciones::xy_write(&bump, &mut f_xy);
  }

  uv_file = funciones::file_name_inc(&uv_file);
  {
    let mut f_uv = BufWriter::new(std::fs::File::create(&uv_file).unwrap());
  ...
  loop {
    ...
    let test = if bump.anew != 0.0 {
      (bump.anew - bump.aold).abs() / bump.anew
    } else {
      0.0
    };
