          ...
          if 0 < i_val {
            ...
            for iqq in 0..NNODES {
              if iqq == 0 || iqq == 1 || iqq == 3 {
                ...
                let (bbb, _bbx_, _bby_) = qbf(x, y, it, iqq, &bump.node, &bump.xc, &bump.yc);
                let j_val = bump.indx[ipp * 2];
                if j_val != 0 {
                  let jj = bump.iline_inv[j_val as usize];
                  bump.gr[((ii - 1) as usize) * bump.my + ((jj - 1) as usize)] += bb * bbb * ar;
                }
              }
            }
          }
      ...
      for iq in 0..NNODES {
        ...
        for iqq in 0..NNODES {
          ...
          if 0 < ju {
            ...
            if 0 < iver {
              bump.res[(iver - 1) as usize] += ar * bb * bbb * unx[1] * bump.g[(ju - 1) as usize];
            }
            if 0 < iprs {
              bump.res[(iprs - 1) as usize] += ar * bbx * bbl * bump.g[(ju - 1) as usize];
            }
          } else if ju == -1 {
          ...
          if 0 < jv {
            if 0 < ihor {
              bump.res[(ihor - 1) as usize] += ar * bb * bbb * uny[0] * bump.g[(jv - 1) as usize];
            }
            if 0 < iver {
            ...
            if 0 < iprs {
              bump.res[(iprs - 1) as usize] += ar * bby * bbl * bump.g[(jv - 1) as usize];
            }
          } else if jv == -1 {
          ...
          if 0 < jp {
            if 0 < ihor {
              bump.res[(ihor - 1) as usize] -= ar * bx * bbbl * bump.g[(jp - 1) as usize];
            }
            if 0 < iver {
              bump.res[(iver - 1) as usize] -= ar * by * bbbl * bump.g[(jp - 1) as usize];
            }
          }
        }
      }
